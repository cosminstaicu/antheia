<?php
namespace Antheia\Framework\Classes\Table;
...
use Antheia\Framework\Classes\Table\Plain\Row;
...
class TablePlain extends AbstractClass implements HtmlCode, HtmlId {
	...
	public function addRow(TableRow $row = NULL):TableRow {
		...
		if (!is_a($row, 'Antheia\Framework\Classes\Table\Plain\Row')) {
			throw new Exception(
				'Only Antheia\Framework\Classes\Table\Plain\Row instances allowed'
			);
		}
		$this->rows[] = $row;
		return $row;

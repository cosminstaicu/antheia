 ...
 */
function jsf_forms_updateStatus(id) {
	if (id === "") {
		// no id defined
		return false;
	}
	...
	for (i = 0; i < 10; i++) {
		if (statusIcon.classList.contains("jsf_form-item")) {
			break;
		}
	...
	statusIcon.classList.add("jsf-status");
	if (window[input.dataset.validate] === undefined) {
		throw new Error("Unknown function: " + input.dataset.validate + "()");
	}
	if (window[input.dataset.validate]()) {
		// valid field
		statusIcon.classList.remove("jsf-invalid");
		statusIcon.classList.add("jsf-valid");
	} else {
		// invalid field
		statusIcon.classList.remove("jsf-valid");
		statusIcon.classList.add("jsf-invalid");
	}
}
/**
 ...
 */
function jsf_forms_updateValue(id, value, readableValue) {
	let button = null;
	...
	if (inputType === "input-hidden") {
		...
		if (checkedInput.dataset.jsfType !== undefined) {
			// this is a special input
			inputType += "-" + checkedInput.dataset.jsfType;
		}
	}
	switch (inputType) {
		case "input-hidden":
		...
		case "input-hidden-date":
			...
			if (value == button.dataset.undefinedDate) {
				...
				let month = jsf_text_months[value.slice(4,6)];
		...
		case "select":
			...
			jsf_inputSelect_updateInfo(element);
			break;
		case "textarea":

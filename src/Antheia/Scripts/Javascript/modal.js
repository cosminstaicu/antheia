...
class jsf_modal {
	...
	constructor () {
		...
		this.htmlElement.classList.add('jsf_modal');
		switch (jsf_theme_backdrop) {
			case "simple":
				this.htmlElement.classList.remove("jsf-blur");
				break;
			case "blur":
				this.htmlElement.classList.add("jsf-blur");
				break;
			default:
		}
		...
		this.panel.classList.add("jsf_panel");
		...
		this.content.classList.add("jsf_modal-content");
	...
	#setElementAlign(element, align) {
		element.classList.remove('jsf-left');
		element.classList.remove('jsf-center');
		element.classList.remove('jsf-right');
		element.classList.add('jsf-' + align);
	}
	/**
	...
	setHeader(headerCode) {
		...
		if (this.header === null) {
			...
			this.header.classList.add("jsf_modal-header");
	...
	setFooter(footerCode) {
		...
		if (this.footer === null) {
			...
			this.footer.classList.add("jsf_modal-footer");
	 ...
	 */
	show() {
		this.htmlElement.classList.add("jsf-hidden");
		document.body.append(this.htmlElement);
		setTimeout(() => {
			this.htmlElement.classList.remove("jsf-hidden");
		}, 50);
	}
	/**
	...
	startLoading(params) {
		...
		this.panel.classList.add("jsf-loading");
		if (hideHeader) {
			this.panel.classList.add("jsf-no-header");
		}
		if (hideFooter) {
			this.panel.classList.add("jsf-no-footer");
		}
	}
	/**
	 ...
	 */
	stopLoading() {
		this.panel.classList.remove("jsf-loading");
		this.panel.classList.remove("jsf-no-header");
		this.panel.classList.remove("jsf-no-footer");
	}
	/**
	 * Removes the html element from the body of the page
	 */
	hide() {
		if (this.htmlElement.classList.contains("jsf-hidden")) {
			return false;
		}

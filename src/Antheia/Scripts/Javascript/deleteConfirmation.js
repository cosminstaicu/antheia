 ...
 */
function jsf_deleteConfirmation(element) {
	let modal = new jsf_modal();
	modal.setHeader(element.dataset.textInfo);
	// the visible input
	let inputText = document.createElement("input");
	inputText.type = "text";
	inputText.name = "jsf_delete_" + (new Date()).getTime();
	inputText.value = "";
	...
	submit.onclick = () => {
		...
		if (inputValue !== element.dataset.textTemplate) {
			jsf_alert.quickError(element.dataset.textError, () => {
				inputText.value = "";
				inputText.focus();
			});
			return false;

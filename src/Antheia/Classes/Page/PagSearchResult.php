 ...
 */
class PagSearchResult extends PageEmpty {
	...
	public function __construct() {
		...
		$this->setListType(PagSearchResult::LIST_TYPE_ACCORDION);
		$this->results = [];
	 ...
	 * of the constants like jsc_page_searchResult::LIST_TYPE_##
	 */
	...
	public function addInput(
		...
		if ($input->getHtmlId() === NULL) {
			$input->setHtmlId('jsf_search-filter-id'.$this->uniqueIdCounter++);
		}
	...
	public function getHtml():string {
		switch ($this->listType) {
			case PagSearchResult::LIST_TYPE_ACCORDION:
				$results = new SearchViewAccordion();
				break;
			case PagSearchResult::LIST_TYPE_CARDS:
				$results = new SearchViewCards();
				break;
			case PagSearchResult::LIST_TYPE_TABLE:
		...
		$javascript = 'jsf_search_total = '.count($this->results).';
			function jsf_search_renderSelection() {
		...
		$code = '<div id="jsf_search-pages"><table><tr><td>';
		if ($this->currentPage > 1) {
			$code .= '<button class="jsf-back" onClick="jsf_search_changePage('
			.($this->currentPage-1).')">'.$previousIcon->getHtml().'</button>';
		}
		$code .= '
			</td><td>'.Texts::get('PAGE').' </td>
			<td><input id="jsf_search-input-page" type="number" min="1"
			value="'.$this->currentPage.'"
			onkeypress="jsf_search_pageInputUpdated(event,'
			.$this->pages.')"></td><td>'.Texts::get('OF').' </td>
			<td> '.$this->pages.'</td><td>';
		if ($this->currentPage < $this->pages) {
			$code .= '<button class="jsf-forward" onClick="jsf_search_changePage('

<?php
use Antheia\Framework\Classes\Input\SearchResponse\SearchResponseItem;
use Antheia\Framework\Classes\Input\SearchResponse\SearchResponseList;
/**
...
foreach ($values as $index => $answer) {
	...
	if (stripos($answer, $_POST['value']) !== false) {
		$item = new SearchResponseItem();
		$item->setNameValue(htmlspecialchars($answer), $index);
		$list->addItem($item);
	}
}
echo $list->getHtml();
